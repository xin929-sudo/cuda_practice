...
__global__ void cuda_sgemm(float *A_ptr, float *B_ptr, float *C_ptr, const int M, const int N, const int K){
    ...
    // 用于声明缓存 shared Memory 数据的局部寄存器
    register float reg_a;
    register float reg_b[NUM_PER_THREAD];
    ...
    for (int s = 0; s < K; s += K_NUM_PER_BLOCK) {
       ...
       如果 i 在外层，GPU 的执行顺序是：
        ...
        i = 0 时，内层 k 跑 32 次。此时从共享内存读取了 a_shared[ty][0] 到 a_shared[ty][31] 一整行数据，算出了 temp[0]。

        i = 1 时，内层 k 又跑 32 次。此时又把 a_shared[ty][0] 到 a_shared[ty][31] 这同一行数据从共享内存里重新读了一遍，算出了 temp[1]！
       */ 
        ...
        for (int k = 0; k < K_NUM_PER_BLOCK; ++k) {
            ...
            // #pragma unroll
            // for (int i = 0; i < NUM_PER_THREAD; i++) {
            //     reg_b[i] = b_shared[k][tx * NUM_PER_THREAD + i];
            // }
            FETCH_FLOAT4(reg_b) = FETCH_FLOAT4(b_shared[k][tx * NUM_PER_THREAD]);
            // 开始计算
            #pragma unroll
            for (int i = 0; i < NUM_PER_THREAD; i++) {
                temp[i] += reg_a * reg_b[i];
            }
        }

            //         for (int k = 0; k < K_NUM_PER_BLOCK; k++) {

            //             // 从共享内存读取A 的一个元素到寄存器（该元素将被接下来的 4 次 乘法复用
            //             reg_a = a_shared[ty][k];

            //             // 从共享内存读取 B 的 4 个元素到寄存器数组
            //             // #pragma unroll
            //             // for (int i = 0; i < NUM_PER_THREAD; i++) {
            //             //     reg_b[i] = b_shared[k][tx * NUM_PER_THREAD + i];
            //             // }
...
int main()
{
    ...
    printf("my_sgemm_v4_using_float4_and_register.cu\n");
