...
__global__ void cuda_sgemm(float *A_ptr, float *B_ptr, float *C_ptr, const int M, const int N, const int K){
    ...
    // 1. 撤销外层的 if，让 Block 内【所有线程】都必须进入这个循环
    for (int s = 0; s < K; s += blockDim.x) {

        // 2. 搬运 A 时的精准边界检查
        // 条件：y 没有超出总行数 M，且当前要读取的列 (s + threadIdx.x) 没有超出总列数 K
        if (y < M && (s + threadIdx.x) < K) {
            a_shared[threadIdx.y][threadIdx.x] = A_ptr_start[threadIdx.y * K + threadIdx.x + s];
        } else {
            a_shared[threadIdx.y][threadIdx.x] = 0.0f; // 越界的地方填 0，这样相乘时 temp 不受影响
        }

        // 3. 搬运 B 时的精准边界检查
        // 条件：x 没有超出总列数 N，且当前要读取的行 (s + threadIdx.y) 没有超出总行数 K
        if (x < N && (s + threadIdx.y) < K) {
            b_shared[threadIdx.y][threadIdx.x] = B_ptr_start[(threadIdx.y + s) * N + threadIdx.x];
        } else {
            b_shared[threadIdx.y][threadIdx.x] = 0.0f; // 越界的地方填 0
        }

        // 4. 所有线程都在这里集合！绝不会有人漏掉！
        __syncthreads();

        // 5. 计算环节，因为越界的数据已经被填了 0，所以随便加，不会算错
    ...
    // 6. 最后一步：只有真正在 C 矩阵范围内的合法线程，才允许把结果写回去
    if (x < N && y < M) {
        C_ptr[y * N + x] = temp;
    }
}
...
int main()
{
    ...
    printf("my_sgemm_v1_shared_memory\n");
    ...
    cuda_sgemm<BLOCK><<<grid, block>>>(matrix_device_A, matrix_device_B, matrix_device_C, m, n, k);
